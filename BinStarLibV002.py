    ...
    def __init__(self,StarIdentifierDD,View):
        ...
        for recordindex in range(1,nrecords):
            ...
            if fields[0] == StarIdentifierDD:
                print "In first if, fields[1]",fields[1]
                if fields[1] == View:
...
def DrawOrbitPlotV0(StarIdentifierDD, time_fine,X_fine,Y_fine,time_coarse,X_coarse,Y_coarse,Observations, \
    X_Fit,Y_Fit,X_Fit_Bound,Y_Fit_Bound,SepCoefs,PACoefs,params,CallNumber):
    import numpy as np
...
###############       
    if CallNumber <= 2:    
        ...
        pl.xlim(params.X0*params.DIST,params.X1*params.DIST)
        pl.xticks(np.linspace(params.X0*params.DIST,params.X1*params.DIST, params.NX, endpoint=True), \
            np.round(np.linspace(params.X0*params.DIST,params.X1*params.DIST, params.NX, endpoint=True),0))
        pl.xticks()
        ...
        pl.ylim(params.Y0*params.DIST,params.Y1*params.DIST)
        pl.yticks(np.linspace(params.Y0*params.DIST,params.Y1*params.DIST, params.NY, endpoint=True), \
            np.round(np.linspace(params.Y0*params.DIST,params.Y1*params.DIST, params.NY, endpoint=True),0))
        ...
        print "ax2=",ax2
        pl.suptitle(StarIdentifierDD,fontsize=8, fontweight='bold')
        #Plot origin lines
